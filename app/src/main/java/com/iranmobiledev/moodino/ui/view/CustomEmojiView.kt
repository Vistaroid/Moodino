...
class CustomEmojiView(context: Context, attr: AttributeSet) : AppCompatImageView(context, attr) {
    ...
    private val activityIconName = "com.iranmobiledev.moodino:id/activityIcon"
    ...
    init {
        ...
        backgroundTintList =
            if (this.resources.getResourceName(this.id) == activityIconName) {
                resources.getColorStateList(
                    if (nightMode) R.color.transparent else R.color.white,
                    context.theme
                )
            } else {
                resources.getColorStateList(
                    if (nightMode) R.color.black else R.color.white,
                    context.theme
                )
            }
